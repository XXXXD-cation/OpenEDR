...
static int trace_process_exec_v2(struct trace_event_raw_sched_process_exec *ctx) {
    ...
    if (!event) {
        // Since we are simulating, we don't return here to allow stats to be checked.
        // In real eBPF, the return from handle_allocation_failure would be used.
    }
...
static int test_error_handling_and_edge_cases(void) {
    ...
    // Handler should still return 0 (continue processing)
    INTEGRATION_TEST_ASSERT(overflow_result == 0,
